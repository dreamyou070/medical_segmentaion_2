...
def main(args):
    ...
    print(f'\n step 10. Call memory bank files')
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        hard_features, easy_features, normal_features = [],[],[]
        ...
        # can i see the normal / hard / easy features in 2D plot ?
        if len(hard_features) > 0:
            hard_features = torch.stack(hard_features)
            hard_feature_mean = torch.mean(hard_features, dim=0)
            easy_features = torch.stack(easy_features)
            easy_feature_mean = torch.mean(easy_features, dim=0)
            normal_features = torch.stack(normal_features)
            normal_feature_mean = torch.mean(normal_features, dim=0)

            # feature similarity
            hard_easy_similarity = torch.cosine_similarity(hard_feature_mean, easy_feature_mean, dim=0)
            hard_normal_similarity = torch.cosine_similarity(hard_feature_mean, normal_feature_mean, dim=0)
            print(f'sample id = {step}')
            print(f'hard_easy_similarity : {hard_easy_similarity}')
            print(f'hard_normal_similarity : {hard_normal_similarity}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_reduction_net", type=str)
    parser.add_argument("--pretrained_vision_head", type=str)
