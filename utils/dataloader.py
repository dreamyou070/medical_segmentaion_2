...
class test_dataset_ori:
    ...
    def load_data(self):
        ...
        return image, gt, name, np.array(image_for_post), org_img
...
class test_dataset:
    ...
    def __init__(self, image_root, gt_root, testsize, unet_transform, condition_transform):
        self.testsize = testsize

        # [1] image and gt directory
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)

        # [2] image and gt transform
        self.unet_transform = unet_transform
        self.condition_transf
        self.size = len(self.images)
        self.index = 0
