...
from peft_origin.utils import INCLUDE_LINEAR_LAYERS_SHORTHAND
...
class BaseTuner(nn.Module, ABC):
    ...
    The easiest is to check what is done in the `peft_origin.tuners.lora.LoraModel` class.
    ...
    Attributes:
        ...
        peft_config (`Union[`PeftConfig`, dict[str, PeftConfig]]`):
            ...
            dictionary with a key `adapter_name` and a value of that peft_origin config.
        config (`dict[str, Any]`):
    ...
    def _prepare_adapter_config(self, peft_config: PeftConfig, model_config: dict) -> PeftConfig:
        ...
        Check out `peft_origin.tuner.lora.LoraModel._prepare_adapter_config` for an example.
    ...
    def _prepare_model(self, peft_config: PeftConfig, model: nn.Module):
        ...
        See `peft_origin.tuner.lora.LoraModel._prepare_model` for an example.
    ...
    def _create_and_replace(
        ...
        Check `peft_origin.tuners.lora.LoraModel._create_and_replace` for an example.
    ...
    def _mark_only_adapters_as_trainable(self, model: nn.Module):
        ...
        Check `peft_origin.tuners.lora.LoraModel._mark_only_adapters_as_trainable` for an example.
        """
        ...
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str):
        ...
        hood by `peft_origin.mapping.get_peft_model` if a non-prompt tuning adapter class is passed.
