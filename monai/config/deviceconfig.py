...
def print_config(file=sys.stdout):
    ...
    print(f"MONA flags: HAS_EXT = {HAS_EXT}, USE_COMPILED = {USE_COMPILED}, USE_META_DICT = {USE_META_DICT}")
    print(f"MONA rev id: {monai.__revision_id__}")
    username = getpass.getuser()
    masked_file_path = re.sub(username, "<username>", monai.__file__)
    print(f"MONA __file__: {masked_file_path}", file=file, flush=True)
...
def print_debug_info(file: TextIO = sys.stdout) -> None:
    ...
    print("Printing MONA config...", file=file, flush=True)
    print("================================", file=file, flush=True)
    print_config(file)
