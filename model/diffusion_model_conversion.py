...
def renew_attention_paths(old_list, n_shave_prefix_segments=0):
    ...
    for old_item in old_list:
        ...
        mapping.append({"old": old_item, "new": new_item})
    return mapping
def assign_to_checkpoint(paths, checkpoint, old_checkpoint, attention_paths_to_split=None,
...
def convert_ldm_unet_checkpoint(checkpoint, config):
    ...
    input_blocks = {
        layer_id: [key for key in unet_state_dict if f"input_blocks.{layer_id}." in key] for
        layer_id in range(num_input_blocks)}
