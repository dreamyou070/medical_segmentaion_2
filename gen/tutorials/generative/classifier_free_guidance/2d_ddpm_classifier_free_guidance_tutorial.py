...
#
# This tutorial illustrates how to use MONA for training a denoising diffusion probabilistic model (DDPM)[1] to create synthetic 2D images using the classifier-free guidance technique [2] to perform conditioning.
#
#
...
# In this tutorial, we will train our models on the MedNIST dataset available on MONA
...
# Here we specify the "unconditional" class with the value `-1` with a probability of training on unconditional being 15%. Specified in the following line using MONA's RandLambdad:
#
...
# At this step, we instantiate the MONA components to create a DDPM, the UNET, the noise scheduler, and the inferer used for training and sampling. We are using
